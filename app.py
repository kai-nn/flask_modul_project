...
import datetime, os, base64, math
...
def test():
    return render_template('test.html', title='Тест', access=get_access()[0], menu=get_access()[1])
...
def authorization():
    ...
    if request.method == 'POST':
        ...
        if temp_login is None:
            # print('отсутствует')
            flash('Пользователь не указан.', 'notifRed')
            return render_template('authorization.html', title='Вход', access=get_access()[0], menu=get_access()[1])

        if temp_login.psw != pas:
            # print('пароль не соответствует')
            flash('Пароль указан не верно.', 'notifRed')
            return render_template('authorization.html', title='Вход', access=get_access()[0], menu=get_access()[1])

        access = Shtat.query.filter_by(user=temp_login).first()

        if access is None:
            # print('Пользователь еще не принят в штат.')
            flash(f"Пользователь {login} еще не принят в штат.", 'notifYellow')
            return render_template('authorization.html', title='Вход', access=get_access()[0], menu=get_access()[1])

        session['username'] = login

        # print('Осуществлен вход: ', login, user_shtat.role_in_shtat.role)
        flash(f"Приветствую, {login}", "notifGreen")
...
def mail_send(addr_to, message):
    ...
    # addr_from = "hydromash.bpzmp@gmail.com"  # Адресат
    # password = "pikinbpzmp"  # Пароль

    addr_from = "kai.nn@mail.ru"                    # Адресат
    password  = "hrenovina"                         # Пароль
